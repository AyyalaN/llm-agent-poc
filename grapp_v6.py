# Gradio UI that talks to A2A agents via the official SDK, with:
# - metadata-aware routing (target_agent / relay boolean)
# - messages-only toggle for the right pane
# - final-initiator reply shown in the left chat for each request

from __future__ import annotations

import os
from dataclasses import dataclass, field
from typing import Any, Dict, List, Tuple, Optional
from uuid import uuid4

import gradio as gr
import httpx

# A2A SDK imports (modern client path)
from a2a.client import A2ACardResolver
from a2a.client.client import ClientConfig
from a2a.client.client_factory import ClientFactory
from a2a import Role, Message, Task, TaskStatusUpdateEvent, TaskArtifactUpdateEvent
from a2a.utils.message import get_message_text

# -----------------------------
# Data classes
# -----------------------------
@dataclass
class AgentHandle:
    key: str
    base_url: str
    color: str
    httpx_client: httpx.AsyncClient
    card: Any
    client: Any

@dataclass
class RelayEvent:
    idx: int
    from_agent: str
    to_agent: str
    kind: str            # "message" | "status" | "artifact" | "task"
    text: str
    raw: Any = None
    meta: dict | None = None

@dataclass
class RequestThread:
    request_id: str
    initiator: str
    prompt: str
    chat_log: List[Tuple[str, str]] = field(default_factory=list)
    relay_events: List[RelayEvent] = field(default_factory=list)
    final_initiator_reply: str | None = None

@dataclass
class AppState:
    agents: Dict[str, AgentHandle] = field(default_factory=dict)
    requests: Dict[str, RequestThread] = field(default_factory=dict)
    selected_request_id: Optional[str] = None

# -----------------------------
# Setup
# -----------------------------
COLOR_MAP = {"claims": "#2563eb", "mr": "#059669"}
NAME_MAP = {"claims_agent": "claims", "mr_agent": "mr", "claims": "claims", "mr": "mr"}
MAX_HOPS = 12

async def build_agent(key: str, base_url: str, color: str) -> AgentHandle:
    httpx_client = httpx.AsyncClient(timeout=None)
    resolver = A2ACardResolver(httpx_client=httpx_client, base_url=base_url)
    card = await resolver.get_agent_card()
    factory = ClientFactory(config=ClientConfig(streaming=True, httpx_client=httpx_client))
    client = await factory.create(card)
    return AgentHandle(key=key, base_url=base_url, color=color, httpx_client=httpx_client, card=card, client=client)

async def close_agent(agent: AgentHandle):
    try:
        await agent.client.close()
    except Exception:
        pass
    try:
        await agent.httpx_client.aclose()
    except Exception:
        pass

# -----------------------------
# Rendering helpers
# -----------------------------
def html_color_span(text: str, color: str) -> str:
    return f"<div style='border-left:6px solid {color}; padding:8px 10px; margin:6px 0; background:#f8f8f8; border-radius:8px;'>{gr.utils.sanitize_html(text)}</div>"

def render_relay(events: List[RelayEvent], color_map: Dict[str, str], messages_only: bool = False) -> str:
    filtered = []
    for ev in events:
        if messages_only and ev.kind != "message":
            continue
        filtered.append(ev)
    if not filtered:
        return "<i>No relayed messages for this request (with current filter).</i>"

    blocks = []
    for ev in filtered:
        head = f"<b>{ev.from_agent} → {ev.to_agent}</b> <small style='opacity:.7'>[{ev.kind}]</small>"
        body = gr.utils.sanitize_html(ev.text or "")
        blocks.append(f"<div style='margin-bottom:8px;'>{html_color_span(head + '<br/>' + body, color_map.get(ev.from_agent, '#999'))}</div>")
    return "<div>" + "".join(blocks) + "</div>"

# -----------------------------
# Streaming + relay
# -----------------------------
def _normalize_relay_flag(v: Any) -> bool:
    if isinstance(v, bool):
        return v
    if isinstance(v, str):
        return v.strip().lower() not in {"no", "false", "never", "0"}
    return True

async def stream_once_and_collect(agent: AgentHandle, user_text: str) -> tuple[str, List[RelayEvent], bool, dict]:
    msg = Message(role=Role.user, parts=[{"kind": "text", "text": user_text}], messageId=uuid4().hex)

    collected: List[RelayEvent] = []
    terminal = False
    agent_reply = ""
    route_hint: dict = {}

    async for ev in agent.client.send_message(msg):
        if isinstance(ev, Message):
            agent_reply = get_message_text(ev) or ""
            meta = getattr(ev, "metadata", None) if hasattr(ev, "metadata") else None
            if isinstance(meta, dict):
                route_hint = {
                    "relay": _normalize_relay_flag(meta.get("relay", True)),
                    "audience": meta.get("audience"),
                    "target_agent": meta.get("target_agent") or meta.get("delegateTo"),
                }
            collected.append(RelayEvent(
                idx=len(collected), from_agent=agent.key, to_agent="", kind="message", text=agent_reply, raw=ev, meta=route_hint or None
            ))
            break

        if isinstance(ev, tuple) and len(ev) == 2:
            task, update = ev
            if isinstance(update, TaskStatusUpdateEvent):
                state_name = getattr(update.status, "state", None)
                is_final = bool(getattr(update, "final", False))
                text = getattr(update.status, "message", "") or ""
                collected.append(RelayEvent(
                    idx=len(collected), from_agent=agent.key, to_agent="", kind="status", text=f"{state_name}: {text}", raw=update
                ))
                if is_final and str(state_name).lower() in {"taskstate.completed", "completed", "taskstate.failed", "failed"}:
                    terminal = True
                    break
            elif isinstance(update, TaskArtifactUpdateEvent):
                preview = ""
                try:
                    if update.artifact and update.artifact.parts:
                        part0 = update.artifact.parts[0]
                        if isinstance(part0, dict):
                            preview = part0.get("text", "") or ""
                except Exception:
                    pass
                collected.append(RelayEvent(
                    idx=len(collected), from_agent=agent.key, to_agent="", kind="artifact", text=preview, raw=update
                ))
            elif update is None and isinstance(task, Task):
                collected.append(RelayEvent(
                    idx=len(collected), from_agent=agent.key, to_agent="", kind="task", text=f"task {task.id} updated", raw=task
                ))

    return agent_reply, collected, terminal, route_hint

async def relay_by_default(state: AppState, request: RequestThread) -> None:
    a = state.agents[request.initiator]
    bkey = next(k for k in state.agents.keys() if k != request.initiator)
    b = state.agents[bkey]

    current_speaker = a
    next_peer = b
    user_in = request.prompt

    for hop in range(MAX_HOPS):
        reply, events, terminal, route_hint = await stream_once_and_collect(current_speaker, user_in)
        for ev in events:
            ev.to_agent = next_peer.key
        request.relay_events.extend(events)

        if reply and current_speaker.key == request.initiator:
            request.final_initiator_reply = reply

        if terminal:
            break

        relay_ok = True
        if route_hint:
            if route_hint.get("audience") == "user":
                relay_ok = _normalize_relay_flag(route_hint.get("relay", False))
            else:
                relay_ok = _normalize_relay_flag(route_hint.get("relay", True))
        if not relay_ok:
            break

        if not reply:
            break
        user_in = reply

        # metadata-driven routing
        next_key = None
        if route_hint and route_hint.get("target_agent"):
            next_key = NAME_MAP.get(str(route_hint["target_agent"]).strip().lower())
        if next_key and next_key in state.agents:
            current_speaker = state.agents[next_key]
            other_key = next(k for k in state.agents.keys() if k != next_key)
            next_peer = state.agents[other_key]
        else:
            current_speaker, next_peer = next_peer, current_speaker

# -----------------------------
# Gradio UI + Handlers
# -----------------------------
async def on_connect(claims_base: str, mr_base: str, app_state: AppState):
    for h in list(app_state.agents.values()):
        await close_agent(h)
    app_state.agents.clear()

    claims = await build_agent("claims", claims_base, COLOR_MAP["claims"])
    mr = await build_agent("mr", mr_base, COLOR_MAP["mr"])
    app_state.agents = {"claims": claims, "mr": mr}
    return gr.update(value="Connected ✓"), gr.update(choices=["claims", "mr"], value="claims")

async def on_submit(prompt: str, initiator: str, app_state: AppState,
                    chat: List[Tuple[str, str]], relay_html: str,
                    messages_only: bool):
    if not prompt.strip():
        return gr.update(), gr.update(), gr.update()

    request_id = uuid4().hex
    thread = RequestThread(request_id=request_id, initiator=initiator, prompt=prompt)
    app_state.requests[request_id] = thread
    app_state.selected_request_id = request_id

    chat = chat + [(prompt, "")]
    await relay_by_default(app_state, thread)

    chat[-1] = (prompt, thread.final_initiator_reply or "")
    relay_html = render_relay(thread.relay_events, COLOR_MAP, messages_only=messages_only)
    return chat, relay_html, request_id

def on_select_chat(evt: gr.SelectData, app_state: AppState, chat: List[Tuple[str, str]], messages_only: bool):
    selected_user_text = ""
    try:
        selected_user_text = evt.value[0]
    except Exception:
        pass

    chosen = None
    for r in app_state.requests.values():
        if r.prompt == selected_user_text:
            chosen = r
    if not chosen:
        return gr.update()

    app_state.selected_request_id = chosen.request_id
    return gr.update(value=render_relay(chosen.relay_events, COLOR_MAP, messages_only=messages_only))

def on_toggle_messages_only(messages_only: bool, app_state: AppState):
    rid = app_state.selected_request_id
    if not rid or rid not in app_state.requests:
        return gr.update()
    thread = app_state.requests[rid]
    return gr.update(value=render_relay(thread.relay_events, COLOR_MAP, messages_only=messages_only))

# -----------------------------
# Build UI
# -----------------------------
def build_ui():
    with gr.Blocks(css="""
    .left-col {min-height: 560px;}
    .right-col {min-height: 560px; background: #fff;}
    """) as demo:
        app_state = gr.State(AppState())

        with gr.Row():
            gr.Markdown("### A2A Relay Dashboard (LangGraph × A2A SDK)")

        with gr.Row():
            claims_url = gr.Textbox(label="Claims Agent Base URL", value=os.getenv("CLAIMS_BASE_URL", "http://localhost:10000"))
            mr_url = gr.Textbox(label="MR Agent Base URL", value=os.getenv("MR_BASE_URL", "http://localhost:10001"))
            connect_btn = gr.Button("Connect")
            status = gr.Markdown("", elem_id="status")

        with gr.Row():
            with gr.Column(scale=1, elem_classes="left-col"):
                initiator = gr.Dropdown(choices=["claims", "mr"], value="claims", label="Initiator Agent")
                chat = gr.Chatbot(label="Chat with Initiator", height=500, type="tuple")
                prompt = gr.Textbox(label="Your prompt", placeholder="e.g., What\'s the status of C-2002?")
                send = gr.Button("Send")

            with gr.Column(scale=1, elem_classes="right-col"):
                with gr.Row():
                    gr.Markdown("**Inter-Agent Relay Trace (for selected request)**")
                    messages_only = gr.Checkbox(value=True, label="Messages only (hide status/artifact)")
                relay_view = gr.HTML("<i>Connect and send a prompt to see relay traffic…</i>", height=500)
                selected_request = gr.Textbox(visible=False)

        connect_btn.click(on_connect, [claims_url, mr_url, app_state], [status, initiator])
        send.click(on_submit, [prompt, initiator, app_state, chat, relay_view, messages_only], [chat, relay_view, selected_request])
        chat.select(on_select_chat, [app_state, chat, messages_only], [relay_view])
        messages_only.change(on_toggle_messages_only, [messages_only, app_state], [relay_view])

    return demo

if __name__ == "__main__":
    demo = build_ui()
    demo.queue().launch()