...
# Adds: "Messages only (hide status/artifact)" toggle to filter the relay view.
import asyncio
import os
...
def build_ui():
    ...
    """) as demo:
        ...
        with gr.Row():
            gr.Markdown("### A2A Relay Dashboard (LangGraph × A2A)")
        ...
        with gr.Row():
            with gr.Column(scale=1, elem_classes="left-col"):
                ...
                prompt = gr.Textbox(label="Your prompt", placeholder="e.g., What's the status of C-1002?")
                send = gr.Button("Send")
        ...
        send.click(on_submit, [prompt, initiator, app_state, chat, relay_view, messages_only],
                   [chat, relay_view, selected_request])
        chat.select(on_select_chat, [app_state, chat, messages_only], [relay_view])
        # NEW: re-render current thread when the filter toggles
